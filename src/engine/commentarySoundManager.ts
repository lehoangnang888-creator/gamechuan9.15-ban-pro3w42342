...
class CommentarySoundManager {
   ...
   */
  getTimelineForInstance(instanceId: number, seed: number = 632585, durationSeconds: number = 120): ScheduledCommentaryEvent[] {
    const validInstId = ((Math.max(1, instanceId) - 1) % 10) + 1;
    
    // Tạo kịch bản biến thể phong phú hàng triệu câu tự động theo seed
    const rawTimeline = generateFullRaceCommentaryTimeline(validInstId, seed, durationSeconds);
